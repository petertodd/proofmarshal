...
use super::{Persist, Unverified, Valid};
...
macro_rules! impl_ints {
    ( $( $t:ident, )* ) => {
        $(
            ...
            unsafe impl<A: ?Sized> Persist<A> for Le<$t> {
                type Error = !;

                #[inline]
                fn verify<'a>(unver: Unverified<'a, Self>, _: &A) -> Result<Valid<'a, Self>, Self::Error> {
...
mod tests {
    ...
    fn nonzero() {
        ...
        let r = <Le<NonZeroU32> as Persist>::verify(unver, &()).unwrap().to_valid_ref();
